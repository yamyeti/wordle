...
def evaluate_agent(env, agent, output_file, specific_word, start_word):
    # Reset environment with the specific word to guess
    env.reset(specific_word=specific_word)

    with open(output_file, "w") as file:
        file.write(f"Specific word to guess: {specific_word}\n")
        file.write(f"Starting guess: {start_word}\t\tValid guesses left: {len(env.possible_words)}\n")

        done = False
        state = env.state.flatten()
        tries = 0  # Initialize tries counter

        # Use start_word as the first guess
        if start_word in env.all_words:
            action = env.all_words.index(start_word)
            word_indices = agent.encode_word(start_word)
            next_state, reward, done, guess_word = env.step(action)
            state = next_state  # Update state
            tries += 1  # Increment tries counter
            file.write(f"Try #1: Guess='{start_word}', Valid guesses left: {len(env.possible_words)}\n")
            if done and reward > 0:
                file.write("\t\t\tWord solved!\n")
                file.write(f"\t\t\tWord '{specific_word}' solved in {tries} tries\n")
        else:
            file.write("Invalid start word provided.\n")
            return

        while not done:
            # Agent decides the next action based on the current state
            action, word_indices = agent.act(state)
            next_state, reward, done, guess_word = env.step(action)
            state = next_state  # Update state
            tries += 1  # Increment tries counter

            # Log each guess and the remaining valid words
            file.write(f"Try #{tries}: Guess='{guess_word}', Valid guesses left: {len(env.possible_words)}\n")

            if done:
                # Check if word was solved
                if reward > 0:  # Assuming reward > 0 means success
                    file.write("\t\t\tWord solved!\n")
                    file.write(f"\t\t\tWord '{specific_word}' solved in {tries} tries\n")
                else:
                    # If not solved successfully, you can log that too
                    file.write("\t\t\tWord not solved.\n")
