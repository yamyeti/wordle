...
class DQNAgent:
    def __init__(self, state_size, action_size, env, start_word_index):
        ...
        self.env = env
        self.start_word_index = start_word_index  # Index of the standard starting guess

    def act(self, state, use_start_word=False):
        if use_start_word and not self.env.guess_history:
            # Use standard starting guess for the first move
...
agent = DQNAgent(state_size, action_size, env, start_word_index=start_word_index)
...
for e in range(total_episodes):
    ...
    use_start_word = True  # use standard starting guess for the first move

    while not done:
        if use_start_word:
            # Use the standard starting guess for first move
            action = start_word_index
            use_start_word = False  # make sure it's only used for the first move
        else:
            # for next moves, use act method
...
def evaluate_agent(env, agent, episodes=100):
    ...
    for e in range(episodes):
        ...
        while not done:
            ...
            next_state, reward, done, _ = env.step(action)  # Apply action, get feedback
            guesses += 1
            state = np.reshape(next_state, [1, agent.state_size])
            last_reward = reward  # Update the last received reward

            if done:
                # if game ended, check if the last reward indicates success
                if last_reward == 3.0:
                    success_count += 1
                    total_guesses_on_success += guesses
                    print(f"Episode {e+1}: Success with {guesses} guesses.")
                else:
                    print(f"Episode {e+1}: Failed to solve after {guesses} guesses.")
...
# prioritize exploitation over exploration during evaluation
agent.epsilon = 0.01
